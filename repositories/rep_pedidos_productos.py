...
def agregar_producto_a_pedido(id_pedido, id_producto_FK, cantidad, precio_unitario):
    """
    ...
    try:
        cursor.execute(
            ...
            VALUES (%s, %s, %s, %s)
            """,
            (id_pedido, id_producto_FK, cantidad, precio_unitario),
        )
        conn.commit()
...
def actualizar_producto_pedido(id_pedido, id_producto_FK, cantidad, precio_unitario):
    """
    ...
    :param precio_unitario: Nuevo precio unitario del producto.
    ...
    try:
        cursor.execute(
            ...
            """,
            (cantidad, precio_unitario, id_pedido, id_producto_FK),
        )
        conn.commit()
...
def obtener_productos_por_pedido(id_pedido):
    ...
    try:
        cursor.execute(
            """
            SELECT pr.id_producto, pr.nombre_producto, pr.precio, pp.cantidad
